		...
		if (jodata.has("pos")) {
			...
			if (!jopos.has("x_range"))
				throw new IllegalArgumentException(
						"The \"x_range\" key does not exist inside \"pos\" in an element of the \"wolf\" key.");
			JSONArray jax = jopos.getJSONArray("x_range");
			if (!jopos.has("y_range"))
				throw new IllegalArgumentException(
						"The \"y_range\" key does not exist inside \"pos\" in an element of the \"wolf\" key.");
			...
			try {
				...
				throw new IllegalArgumentException(
						"Any value of the \"pos\" key in an element of the \"wolf\" key is not a numeric value.");
			}
			if (min_x < 0 || max_x < 0 || max_y < 0 || min_y < 0)
				throw new IllegalArgumentException(
						"To generate the random position of \"wolf\", \"x_range\" and \"y_range\" must be positive values.");
			if (min_x > max_x || max_y < min_y)
				throw new IllegalArgumentException(
						"To generate the random position of \"wolf\", \"x_range\" and \"y_range\" must be values sorted from lowest to highest.");

			Vector2D pos = Vector2D.get_random_vector(min_x, max_x, min_y, max_y);
			return new Wolf(mate_strategy, hunt_strategy, pos);
		} else
			return new Wolf(mate_strategy, hunt_strategy, null);
