...
public class Lion extends Animal{
	...
	private static final double SPEED_PARAM_LION_RAPID = 3.0;
	private static final double SPEED_PARAM_LION_NORMAL = 1.0;

	private static final String GENETIC_CODE = "Lion";

	private Animal _hunt_target;
	private SelectionStrategy _hunting_strategy;

	public Lion(SelectionStrategy mate_strategy, SelectionStrategy hunting_strategy, Vector2D pos) {
	...
	protected Lion(Lion p1, Animal p2) {
		super(p1, p2);
		this._hunt_target = null;
	...
	private void avanzaPosition(double dt) {
		...
		this.moveLion(dt, SPEED_PARAM_LION_NORMAL);
		this._age += dt;
	...
	private void tryHunt(double dt) {
		...
		this.moveLion(dt, SPEED_PARAM_LION_RAPID);
		this._age += dt;
	...
	private void mate() {
		...
		if (this._baby == null) {
			if (Utils._rand.nextDouble(1) >= 0.10) {
				this._baby = new Lion(this, _mate_target);
			}
	...
	private void moveLion(double dt, double in_speed) {
	...
	@Override
	protected Lion clone() {
		Lion a = new Lion(this._mate_strategy, this._hunting_strategy, this._pos);
		return a;
	}
